...
```{r}
min_and_max<-NHANES%>%filter(AgeDecade==" 20-29",Gender=="female")%>%summarize(minimum=min(BPSysAve,na.rm=TRUE),maximum=max(BPSysAve,na.rm=TRUE))
min_and_max
```
...
map_df(n,function(n){
  x<-1:n
  y<-1:sum(x)
  data_frame("n"=n,"s_n"=sum(x),"s_n_2"=sum(y))
})
```
