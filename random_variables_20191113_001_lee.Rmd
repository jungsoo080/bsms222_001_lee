...
Note that statistical textbooks use the Greek letters μ and σ to denote the expected value and standard error, respectively. This is because μ is the Greek letter for  
m, the first letter of mean, which is another term used for expected value. Similarly, σ is the Greek letter for s, the first letter of standard error.
...
```{r}
american_roulette <- rep(c('red', 'black', 'green'), c(18, 18, 2))
mean(american_roulette %in% 'green')
```
...
If Pr(S<0) = 0.01 then Pr(S-E[S]/SE[S] < -E[S]/SE[S])
...
```{r}
S <- replicate(10000, {
    defaults <- sample( c(0,1), 10000, prob=c(1-p, p), replace = TRUE) 
  sum(defaults * loss_per_foreclosure)
})
...
```{r}
10000 * (p * loss_per_foreclosure + (1-p) * 0)
```

4. What is the standard error of S?

```{r}
sqrt(10000) * abs(loss_per_foreclosure) * sqrt(p*(1-p))
```
...
-loss_per_foreclosure*(10000*p - z*sqrt(10000*p*(1-p))) / (10000*(1-p)+z*sqrt(10000*p*(1-p))) /180000
```
